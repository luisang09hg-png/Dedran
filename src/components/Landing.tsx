...
export default function Landing({ onNavigate }: { onNavigate: (page: string) => void }) {
  return (
    <div className="min-h-screen bg-[#06091A]">
      <nav className="sticky top-0 z-50 bg-[#06091A]/80 backdrop-blur-md border-b border-[#1E3354]">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
          <span className="text-2xl font-extrabold bg-gradient-to-r from-[#60A5FA] to-[#24476C] bg-clip-text text-transparent tracking-tight">
            Dedran
          </span>
          <div className="flex items-center gap-4">
            <button onClick={() => onNavigate('sign-in')} className="text-[#A8A9AD] hover:text-[#E6E8E6] transition-colors text-sm font-semibold">
              Sign In
            </button>
            <button
        ...
        <motion.div className="relative max-w-6xl mx-auto px-4" variants={containerVariants} initial="hidden" animate="visible">
          ...
          <motion.div variants={itemVariants} className="grid md:grid-cols-3 gap-5 mb-16">
            {stats.map((stat, idx) => (
              <motion.div
                key={idx}
                className="bg-[#0D1A31] p-6 rounded-2xl border border-[#1E3354] hover:border-[#24476C]/60 transition-all duration-300 animate-float"
                style={{ animationDelay: `${idx * 0.5}s` }}
                whileHover={{ scale: 1.02 }}
              >
                <stat.icon className="w-10 h-10 text-[#60A5FA] mb-4" />
                <h3 className="text-3xl font-extrabold text-[#E6E8E6] mb-1">{stat.value}</h3>
                <p className="text-[#A8A9AD] text-sm font-medium">{stat.label}</p>
              </motion.div>
            ))}
          </motion.div>
          ...
          <motion.div variants={itemVariants}>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {features.map((feat, idx) => (
                <motion.div
                  key={idx}
                  className="bg-[#0D1A31] p-6 rounded-2xl border border-[#1E3354] hover:border-[#60A5FA]/40 transition-all duration-300 cursor-default"
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-xl bg-[#24476C]/30 flex items-center justify-center shrink-0">
                      <feat.icon size={20} className="text-[#60A5FA]" />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-[#E6E8E6] mb-2">{feat.title}</h3>
      ...
      <section className="py-20 bg-gradient-to-r from-[#0A122A] via-[#24476C] to-[#0A122A]">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          >
            ...
            <button
              onClick={() => onNavigate('sign-up')}
              className="px-8 py-3.5 rounded-xl bg-white text-[#0A122A] font-bold hover:shadow-lg hover:shadow-white/20 transition-all"
            >
              Create Free Account
            </button>
          </motion.div>
      ...
      <footer className="bg-[#0A122A] border-t border-[#1E3354] py-12">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div>
              <h3 className="font-extrabold text-lg mb-4 text-white">Dedran</h3>
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-2 text-sm text-[#A8A9AD]">
                  {col.links.map(link => (
                    <li key={link}><a href="#" className="hover:text-[#60A5FA] transition-colors font-inter">{link}</a></li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
          <div className="border-t border-[#1E3354] pt-8 text-center text-sm text-[#A8A9AD]">
